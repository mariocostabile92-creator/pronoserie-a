...
async function pageMondiali(){
  ...
  const partite_giornate = data.partite_giornate || {};
  const fasi = data.fasi_finale || {};
  const letters = Object.keys(gironi).sort();
  const giorni = Object.keys(partite_giornate).sort();
  ...
  html+=`<p style="text-align:center;color:var(--muted);font-size:.75rem;margin-bottom:16px">${data.aggiornamento || "Dati live API-Football"}</p>`;

  function renderPartita(p, accent){
    const score = p.gol_h !== null && p.gol_h !== undefined ? `${p.gol_h} - ${p.gol_a}` : 'vs';
    const statusLabel = p.live ? `<span style="color:#e74c3c;font-size:.7rem;font-weight:800">${p.minuto||""}' LIVE</span>` :
                       p.status === 'FT' ? '<span style="color:#2ecc71;font-size:.7rem">FT</span>' :
                       `<span style="color:var(--muted);font-size:.7rem">${p.data||""} ${p.ora||p.status_it||""}</span>`;
    return `<div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid #0d1b2a;cursor:pointer" onclick="switchLeague('mondiali-2026');navigate('pronostici')">
      <span style="flex:1;text-align:right;font-size:.85rem;font-weight:600">${p.home}</span>
      <span style="min-width:60px;text-align:center;font-weight:800;font-size:.9rem;color:${accent||'var(--green)'}">${score}</span>
  ...
  letters.forEach((g,i)=>{
    ...
    if(partite.length > 0){
      ...
      partite.forEach(p=>{
        html+=renderPartita(p, 'var(--green)');
      });
      html+='</div>';
    }
  ...
  if(giorni.length > 0){
    html+=`<div class="card" style="padding:12px;margin-top:16px;border-left:3px solid var(--green)">`;
    html+=`<h3 style="color:var(--green);margin-bottom:10px">Calendario partite</h3>`;
    giorni.forEach(giorno=>{
      html+=`<h4 style="color:var(--accent);margin:12px 0 6px">${giorno}</h4>`;
      partite_giornate[giorno].forEach(p=>{ html+=renderPartita(p, 'var(--green)'); });
    });
    html+='</div>';
  } else if(letters.length === 0){
    html+=`<div class="card" style="padding:20px;text-align:center">
      <h3 style="color:var(--accent)">Dati Mondiali in caricamento</h3>
      <p style="color:var(--muted);font-size:.85rem">L'API live non ha ancora restituito gironi o calendario. Riprova tra poco.</p>
    </div>`;
  }
  ...
  if(fasiKeys.length > 0){
    ...
    fasiKeys.forEach(fase=>{
      ...
      fasi[fase].forEach(p=>{
        html+=renderPartita(p, 'var(--accent)');
      });
    });
    html+='</div>';
  }
