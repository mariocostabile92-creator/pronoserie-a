...
async function pageClassifica(){
  ...
  if(!data)return `<div class="container">${classificaTabs}<div class="card" style="text-align:center;padding:32px"><h2>Classifica ${leagueLabel}</h2><p style="color:var(--muted);margin:16px 0">Dati in caricamento... Riprova tra qualche secondo.</p><button class="btn btn-blue" onclick="navigate()">Ricarica</button></div></div>`;
  ...
  if(righeClassifica.length){
    ...
    html+='<div style="margin-top:6px;display:flex;gap:8px;justify-content:center;font-size:.7rem;flex-wrap:wrap"><span style="color:var(--green)"> - &#9679; UCL</span><span style="color:var(--yellow)"> - &#9679; UEL</span><span style="color:#9b59b6"> - &#9679; Conf.</span><span style="color:var(--red)"> - &#9679; Retr.</span></div>';
  }else{
    html+='<div style="padding:22px 8px;text-align:center;color:var(--muted);font-size:.9rem">Classifica in attesa dell\\\'inizio della stagione 2026/2027.</div>';
  }
  html+='</div>';
  html+='</div>';
  html+='<div>';
